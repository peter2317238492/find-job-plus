...
所有真实电脑、桌面应用、浏览器窗口、页面 UI、鼠标键盘、截图和登录相关工作，都必须交给专职 `computer-use-agent`。

## 当前能力

| 能力 | 状态 |
| --- | --- |
| Boss 直聘岗位读取与投递准备 | 通过 `computer-use-agent` 请求登录、读取岗位、跳过、投递、消息和市场浏览 |
| 牛客网岗位读取与投递准备 | 通过 `computer-use-agent` 请求登录、读取岗位、投递、消息和市场浏览 |
...
| 本地控制台 | `--gui` 启动 Web 控制台，查看平台、Computer Use 请求、GUI 队列和日志 |
| 测试 | 使用 Node.js 内置 test runner，覆盖过滤、平台请求、Agent 编排、GUI 队列、简历渲染和 Codex 配置 |
...
| `src/agents/computerUseGateway.js` | 唯一的电脑操作请求网关，统一生成 `computer-use-agent` 请求和事件 |
...
| `src/platforms/*` | Boss、牛客平台契约和 computer-use 请求编排 |
| `src/platforms_chrome/*` | 纯解析 helper、LinkedIn profile 维护和历史 Chrome 抽象测试资产 |
...
- `computer-use-agent` 是唯一允许操作电脑、浏览器、桌面 UI、鼠标、键盘和截图的 agent。
- `manager-agent` 负责规划和拆分任务，但不直接使用 Computer Use。
- `computer-use-executor` 执行队列中的可见 GUI 任务，不做策略规划。
- `platform-agent` 只维护平台适配器契约和 request payload。
- `resume-agent`、`chat-agent`、`job-filter-agent`、`idle-market-agent` 不得读取屏幕、点击页面、发送消息或控制浏览器。
...
3. 平台适配器通过 `computerUse.request(...)` 请求登录、读取岗位或执行平台操作。
4. `computer-use-agent` 读取可见页面并返回结构化岗位或消息数据。
...
- 任何真实 GUI 写操作都应由 `computer-use-agent` 执行，不要在非 GUI worker 中直接操作页面。
- 修改简历生成逻辑时，保持“只使用源简历可支持信息”的约束，并补充测试。

## 隐私与安全

- 不在代码中硬编码 API Key，运行时从环境变量读取。
- 不保存平台密码、验证码、Cookie 或浏览器凭据。
- 不绕过验证码、安全验证、风控页或登录流程。
- 不自动执行最终投递、发送消息、上传私有简历、保存公开资料。
- 聊天 agent 只处理低风险常规回复；薪资谈判、证件材料、面试冲突、合同条款等需要用户确认。
- 简历优化提示词要求不编造经历、不泄露无关隐私。
- 所有真实电脑和浏览器动作集中到 `computer-use-agent`，便于审计和人工接管。
- 定制简历、投递方向库和生成 PDF 属于私人数据，默认只保存在本地，并通过 `.gitignore` 排除。
